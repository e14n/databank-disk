...
//
// Copyright 2011,2012,2017 StatusNet Inc.
//
...
var DiskDatabank = function(params) {
    ...
    this.mode = params.mode || 0660 // eslint-disable-line no-octal
        ...
        tryAgain = function() {
            randomString(16, function(err, rs) {
                ...
                if (err) {
                    ...
                    bank.q.enqueue(fs.stat, [dirname], function(err) {
...
DiskDatabank.prototype.create = function(type, id, value, callback) {
    ...
    Step(
        ...
        function(err) {
            if (err) {
                if (fd) {
                    bank.q.enqueue(fs.close, [fd], function() {
                        DiskDatabank.schlock.writeUnlock(filename, function() {
                            callback(err, null)
                        })
                    })
                } else if (schlocked) {
...
DiskDatabank.prototype.save = function(type, id, value, callback)
{
    ...
    Step(
        ...
        function(err) {
            ...
            bank.q.enqueue(fs.ftruncate, [fd, 0], this)
        },
        function(err) {
            var b
            if (err) throw err
    ...
    Step(
        ...
        function(err) {
            ...
            if (err) {
                if (fd) {
                    bank.q.enqueue(fs.close, [fd], function() {
                        DiskDatabank.schlock.readUnlock(filename, function() {
                            callback(err, null)
                        })
                    })
                } else if (schlocked) {
...
DiskDatabank.prototype.search = function(type, criteria, onResult, onCompletion) {
    var bank = this,
        iterator = function(object) {
            if (bank.matchesCriteria(object, criteria)) {
                onResult(object)
            }
        }
    ...
    bank.scan(type, iterator, onCompletion)
}

DiskDatabank.prototype.scan = function(type, onResult, onCompletion) {
    var bank = this,
            ...
            Step(
                ...
                function(err) {
                    ...
                    if (err) {
                        if (fd) {
                            bank.q.enqueue(fs.close, [fd], function() {
                                DiskDatabank.schlock.readUnlock(path, function() {
                                    callback(err, null)
                                })
                            })
                        } else if (schlocked) {
