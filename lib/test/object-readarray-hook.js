                ...
                'and we can create a few people': {
                    ...
                    'and we read a few back': {
                        topic: function(people, Person) {
                            ...
                            Person.readArray(nicknames, function(err, ppl) {
                                called.people = ppl;
                                cb(err, called);
                            });
                        },
                        ...
                        'and the after hook modification happened': function(err, called) {
                            var i = 0;
                            for (i = 0; i < called.people.length; i++) {
                                assert.isObject(called.people[i]);
                                assert.equal(called.people[i].addedByAfter, 23);
                            }
                        }
                    }
                }
